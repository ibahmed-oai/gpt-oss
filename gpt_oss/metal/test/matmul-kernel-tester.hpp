...
public:
  ...
  std::uint32_t num_cols() const { return num_cols_; }
  ...
  std::uint32_t num_tokens() const { return num_tokens_; }
  ...
  std::size_t threadgroup_size() const { return threadgroup_size_; }
  ...
  void TestF32_BF16W(MatMulKernelType mat_mul_kernel_type =
                         MatMulKernelType::DECODE_OPTIMIZED) const {
    Validate(/*vec_size=*/4);

    metal::CommandBuffer command_buffer{command_queue_};
    metal::Buffer input_buffer{device_,
                               num_tokens() * num_cols() * sizeof(float)};
    metal::Buffer weight_buffer{device_, num_rows() * num_cols() *
                                             sizeof(gptoss_bfloat16)};
    metal::Buffer bias_buffer{device_, num_rows() * sizeof(gptoss_bfloat16)};
    metal::Buffer output_buffer{device_,
                                num_tokens() * num_rows() * sizeof(float)};
    metal::Buffer output_buffer_copy{device_,
                                     num_tokens() * num_rows() * sizeof(float)};
    ...
    command_buffer.encode_launch_f32_fill_random(
        ...
        /*output_offset=*/0, num_tokens() * num_cols(), kSeed, /*offset=*/0,
        /*min=*/-1.0f, /*max=*/1.0);
    ...
    command_buffer.encode_launch_bf16_fill_random(
        ...
        /*output_offset=*/0, num_rows() * num_cols(), kSeed + 1, /*offset=*/0,
        /*min=*/-1.0f, /*max=*/1.0);
    ...
    command_buffer.encode_launch_bf16_fill_random(
        ...
        /*output_offset=*/0, num_rows(), kSeed + 2, /*offset=*/0, /*min=*/-1.0f,
        /*max=*/1.0);

    // Fill output buffer with random values.
    command_buffer.encode_launch_f32_fill_random(
        f32_fill_random_fn_,
        /*threadgroup_size=*/0,
        /*max_threadgroups=*/kFillRandomMaxThreadgroups,
    ...
    if (mat_mul_kernel_type ==
      ...
      const float *output_ptr_initial =
          static_cast<const float *>(output_buffer.ptr());
      const float *output_ptr_copy_initial =
          static_cast<const float *>(output_buffer_copy.ptr());
      uint32_t non_zero_count = 0;
      for (size_t t = 0; t < num_tokens(); t++) {
        for (size_t r = 0; r < num_rows(); r++) {
          ASSERT_EQ(output_ptr_initial[t * num_rows() + r],
                    output_ptr_copy_initial[t * num_rows() + r]);
          if (output_ptr_initial[t * num_rows() + r] != 0.0f) {
            non_zero_count++;
          }
        }
      }
      ASSERT_GT(non_zero_count, 0);
    }

    metal::CommandBuffer command_buffer_compute{command_queue_};
    switch (mat_mul_kernel_type) {
    case MatMulKernelType::DECODE_OPTIMIZED:
      Check(gptoss_metal_command_buffer_encode_launch_f32_bf16w_matmul(
                ...
                /*output_offset=*/0, num_tokens(), num_cols(), num_rows()),
    ...
    case MatMulKernelType::PREFILL_QKV_OPTIMIZED:
      Check(
          gptoss_metal_command_buffer_encode_launch_f32_bf16w_dense_matmul_qkv(
              ...
              /*bias_offset=*/0, output_buffer.handle(),
              /*output_offset=*/0, num_tokens(), num_cols(), num_rows()),
          "gptoss_metal_command_buffer_encode_launch_f32_bf16w_dense_matmul_"
          "qkv");
      break;
    ...
    const gptoss_bfloat16 *weight_ptr =
        static_cast<const gptoss_bfloat16 *>(weight_buffer.ptr());
    const gptoss_bfloat16 *bias_ptr =
        static_cast<const gptoss_bfloat16 *>(bias_buffer.ptr());
    const float *output_ptr = static_cast<const float *>(output_buffer.ptr());
    const float *output_ptr_copy =
        static_cast<const float *>(output_buffer_copy.ptr());
    for (size_t t = 0; t < num_tokens(); t++) {
      for (size_t r = 0; r < num_rows(); r++) {
...
private:
  ...
  metal::Function f32_bf16w_dense_matmul_qkv_fn_{
      library_, "gptoss_f32_bf16w_dense_matmul_qkv"};
  metal::Function f32_bf16w_dense_matmul_attn_output_fn_{
      library_, "gptoss_f32_bf16w_dense_matmul_attn_output"};
  metal::Function f32_bf16w_dense_matmul_mlp_gate_fn_{
      library_, "gptoss_f32_bf16w_dense_matmul_mlp_gate"};
